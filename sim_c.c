...
static const Instr4 itab4[] = {
    ...
    {"fmadd",  0x76,1,0,0}, {"fmsub",  0x78,1,0,0},
    ...
    {"frecip", 0x7e,1,1,0x0e}, {"frsqrt", 0x7e,1,1,0x0f},
    ...
    {"bltz",   0xdf,2,4,0x03}, {"bgez",  0xdf,2,4,0x04}, {"bgtz", 0xdf,2,4,0x05}, {"blez", 0xdf,2,4,0x06},
...
/* FPU seed ops (ISA doc "frecip"/"frsqrt"; tables generated by cpu4/gen_fpu_roms.py). */
static uint32_t frecip_seed(uint32_t x) {
    uint32_t sign = x & 0x80000000u, e = (x >> 23) & 0xff, i = (x >> 13) & 0x3ff;
    if (e == 0 && (x & 0x7fffff) == 0) return 0;                 /* zero -> 0 (RTL behaviour) */
    uint32_t ee = (i == 0) ? (254 - e) : (253 - e);
    return sign | ((ee & 0xff) << 23) | ((uint32_t)FRECIP_ROM[i] << 7);
}
static uint32_t frsqrt_seed(uint32_t x) {
    uint32_t e = (x >> 23) & 0xff, m = x & 0x7fffff;
    if (e == 0)   return 0x7f800000u;                            /* zero/denormal -> +inf */
    if (e == 255) return 0;                                      /* inf/NaN -> 0 */
...
#define H  (cc->H)
    ...
    for (int step = 0; step < g_max_steps && g_live > 0; step++, g_cycles++) {
      for (int ci = 0; ci < g_nunits; ci++) {
        ...
        case 0x76: r[rd]=float2bits(bits2float(r[rd])+bits2float(float2bits(bits2float(r[rx])*bits2float(r[ry])))); break; /* fmadd: fadd(rd, fmul(rx,ry)) */
        case 0x78: r[rd]=float2bits(bits2float(r[rd])-bits2float(float2bits(bits2float(r[rx])*bits2float(r[ry])))); break; /* fmsub: fsub(rd, fmul(rx,ry)) */
        /* F1b */
        case 0x7e:
            ...
            else if (subop==0x0e) { r[rd] = frecip_seed(r[rd]); }                              /* frecip */
            else if (subop==0x0f) { r[rd] = frsqrt_seed(r[rd]); }                              /* frsqrt */
