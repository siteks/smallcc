...
# Data 8 or 16 bit. Always signed
#
# State:
#  pc   
#  sp
...
class State:
    ...
    def reset(self):
        ...
        self.H  = 0
    def __repr__(self):
        return 'r0:%04x sp:%04x bp:%04x lr:%04x pc:%04x H:%x' % (
...
class CPU:
    ...
    def step(self, trace=False):
        ...
        elif    i == 'sb':      m.write8(m.read16(s.sp), s.r0); s.sp += 2
        elif    i == 'sw':      m.write16(m.read16(s.sp), s.r0); s.sp += 2
        elif    i == 'add':     s.r0 = m.read16(s.sp) + s.r0; s.sp += 2
        elif    i == 'sub':     s.r0 = m.read16(s.sp) - s.r0; s.sp += 2
        elif    i == 'mul':     s.r0 = m.read16(s.sp) * s.r0; s.sp += 2
        elif    i == 'div':     s.r0 = m.read16(s.sp) / s.r0; s.sp += 2
        elif    i == 'mod':     s.r0 = m.read16(s.sp) % s.r0; s.sp += 2
        elif    i == 'shl':     s.r0 = m.read16(s.sp) << s.r0; s.sp += 2
        elif    i == 'shr':     s.r0 = m.read16(s.sp) >> s.r0; s.sp += 2
        elif    i == 'lt':      s.r0 = m.read16(s.sp) < s.r0; s.sp += 2
        elif    i == 'le':      s.r0 = m.read16(s.sp) <= s.r0; s.sp += 2
        elif    i == 'gt':      s.r0 = m.read16(s.sp) > s.r0; s.sp += 2
        elif    i == 'ge':      s.r0 = m.read16(s.sp) >= s.r0; s.sp += 2
        elif    i == 'eq':      s.r0 = m.read16(s.sp) == s.r0; s.sp += 2
        ...
        if trace:
            ...
            print('%04x %s %-5s %-12s: %s   %s' % (
                oldpc, sins, i, p, s, m.trace ))
