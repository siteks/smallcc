...
Generated by `tests/gen_fpu_vectors.py` from the Python FPU model
(`cpu4/cpu.py`, a port of `cpu4/fpu_model.h`): 92 operand pairs covering
rounding ties, cancellation, overflow, underflow, denormals, infinities,
signed zeros and random bit patterns, run through every float op and folded
into a hash. The same program passes on `sim_c`, the IR interpreter and the
RTL bench (`make -C hw test-sim`), which is what "the RTL is bit-exact with
the spec" means operationally. Regenerate after any change to the float
semantics; the model itself is checked against IEEE single arithmetic on
random normal operands (see the perf doc, §9, for the validation run).
