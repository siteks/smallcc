...
| 0x0e | `frecip rd` | rd = reciprocal seed of float(rd) (see below) |
| 0x0f | `frsqrt rd` | rd = reciprocal-square-root seed of float(rd) (see below) |
| 0x3f | `putchar rd` | write chr(rd & 0xff) to stderr; rd unchanged |

*(47 slots reserved for future hotspot encodings.)*

**Float seeds.** `frecip` and `frsqrt` are table lookups on the leading
mantissa bits; the tables are the specification and are generated by
`cpu4/gen_fpu_roms.py` (`frecip_rom.hex`, `frsqrt_rom.hex`, 1024 × 16 bits
each, also emitted as `fpu_roms.h` for `sim_c`). With `e` the biased
exponent and `m` the 23-bit mantissa of the operand:
...
- `frsqrt`: `p = ~e[0]`; `i = {p, m[22:14]}`; the table holds the normalised
  ...
  Result exponent `(379 − e)/2` when `e` is odd, else `(380 − e)/2`; result
  `{0, exp, rom[i], 7'b0}` (sign ignored). Exponent 0 gives +inf
...
| 0x03 | `bltz rx, imm10` | if signed(rx) < 0: pc += sext10(imm10) |
| 0x04 | `bgez rx, imm10` | if signed(rx) ≥ 0: pc += sext10(imm10) |
| 0x05 | `bgtz rx, imm10` | if signed(rx) > 0: pc += sext10(imm10) |
| 0x06 | `blez rx, imm10` | if signed(rx) ≤ 0: pc += sext10(imm10) |

*(1 slot available.)*

`bltz`/`bgez`/`bgtz`/`blez` test the register as a two's-complement integer.
On an IEEE-754 bit pattern this is the comparison against 0.0 except for
−0.0 (`bltz` taken, `bgez` not) and NaN; the ISA already accepts signed-zero
and NaN deviations for float `eq`/`ne`, and the compiler lowers `x < 0.0f`,
`x <= 0.0f`, `x > 0.0f`, `x >= 0.0f` (either operand order) to these
branches under the same policy. They replace the `zeroN; flt/lts; jnz`
triple with one instruction.
