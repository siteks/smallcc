...
A custom 32-bit RISC-like machine with a dense variable-width encoding. `sim_c`
(`sim_c.c`) is the executable specification: where this document is ambiguous,
what `sim_c` does is what the compiler targets and what the hardware must match.
`cpu4/cpu.py` (run through `cpu4/sim.py`) is an independent Python model of the
same machine, kept in step by hand and used to generate the float conformance
vectors.

## Where an instruction is defined

An instruction has three parts, each with exactly one home. Nothing is
inferred from the others, so a change touches every row that applies.

| Part | Defined in | Copies and consumers |
|---|---|---|
| **Encoding** — mnemonic, format, opcode bits, sub-op, operand order, immediate kind and range, assembler pseudo-ops | `cpu4/isa.py` | **Generated** by `cpu4/gen_isa.py` (`make isa`): `cpu4/isa_table_c.h` (sim_c's assembler and disassembler table), `cpu4/isa_table.py` (`cpu.py`'s decode table `ptable`, `assembler.py`'s pseudo-ops and PC-relative branch set), [cpu4-encoding.md](cpu4-encoding.md). **Checked**, not generated: `hw/rtl/decode.v` case labels (`make -C hw isa-check`). **Imported**: `hw/scripts/imm_range_check.py`. |
| **Semantics** — what the instruction does to registers, memory and `pc` | This document, in the per-format tables below (prose). `sim_c.c`, `run_cpu4()` (executable) | **Hand-written**, one copy each: `sim_c.c` `run_cpu4()` — a `switch` on the first byte, then on the sub-op for F0b/F1b/F3d; `cpu4/cpu.py` `CPU.step()` — an `elif i == 'mnemonic':` chain keyed by the mnemonic the generated table decodes to; `hw/rtl/decode.v` (control signals) and `hw/rtl/cpu4.v` / `alu.v` (datapath). The compiler is a consumer: `emit.c` chooses mnemonics, `legalize.c` knows immediate ranges, `irsim.c` executes the IR-level equivalent. |
| **Floating-point arithmetic** — the bit-exact result of every float op | `cpu4/fpu_model.h` (C, the spec; summarised in "Floating-point semantics" below) | `#include`d by `sim_c.c`, `irsim.c` and `braun.c` (constant folding); **ported line for line** into `cpu4/cpu.py` (`cpu4_fadd` and friends, which `tests/gen_fpu_vectors.py` uses to produce the expected hash); seed ROMs generated by `cpu4/gen_fpu_roms.py` (`frecip_rom.hex`, `frsqrt_rom.hex`, `fpu_roms.h`; the hardware copies the `.hex` files into `hw/rtl/`); the RTL is checked by `tests/cases/floats/fpu_vectors.c`. |

`make isa-check` (part of `make test`) only proves the **encoding** copies are
fresh, and the hardware's `make isa-check` only that `decode.v`'s case labels
agree. Neither can tell whether `sim_c`, `cpu.py` or the RTL *execute* an
instruction, or agree on what it does. That is what the corpus is for:
`make test` (sim_c), `make test_irsim` (IR interpreter), `make -C hw test-sim`
(RTL). `cpu.py` has no corpus harness; it is exercised by `gen_fpu_vectors.py`
for the float ops and by running `cpu4/sim.py` on a program by hand
(`python3 cpu4/sim.py --maxsteps N file.s` prints the same `r0:…` line as
`sim_c`).

### Adding or changing an instruction

In this order, one commit here, then one downstream (see `coordination.md`):

1. **Encoding.** Add or edit the entry in `cpu4/isa.py` (an operand-swap
   pseudo-op goes in `PSEUDOS`). Run `make isa` and commit the regenerated
   `cpu4/isa_table_c.h`, `cpu4/isa_table.py` and `docs/isa/cpu4-encoding.md`
   with it. From this point every assembler accepts the mnemonic and both
   simulators decode it, but neither executes it.
2. **Semantics, prose.** Add the row to the format's table in this document
   (and the "Floating-point semantics" section plus `cpu4/fpu_model.h` if it
   is a float op).
3. **Semantics, `sim_c`.** Add the `case` in `run_cpu4()` in `sim_c.c`. This
   is the executable spec.
4. **Semantics, `cpu.py`.** Add the `elif i == '…':` branch in `CPU.step()`
   in `cpu4/cpu.py` (and the port of any new `fpu_model.h` function). Check
   it against `sim_c` on a program that uses the instruction.
5. **Compiler.** `emit.c` (instruction selection or a peephole),
   `legalize.c` if a new immediate range matters, `braun.c` and `types.c`
   for a builtin, `irsim.c` if a new IR kind is involved.
6. **Corpus test** under `tests/cases/` that executes the instruction;
   `make test` and `make test_irsim` pass.
7. **Downstream.** `hw/rtl/decode.v` — put the mnemonic in a `// mnemonic`
   comment on the case label so `make -C hw isa-check` can find it — and the
   datapath; `make -C hw test-sim`; bump the submodule.
...
**Source of truth:** the opcode assignments in the tables below are a
rendering of `cpu4/isa.py`; the machine-generated per-format tables are in
[cpu4-encoding.md](cpu4-encoding.md). See "Where an instruction is defined"
above for the generation flow and the change checklist.
